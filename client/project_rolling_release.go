...
// RollingReleaseInfo represents the rolling release configuration
...
type RollingReleaseResponse struct {
	RollingRelease RollingReleaseInfo `json:"rollingRelease"`
	ProjectID      string             `json:"-"` // Not in API response
	TeamID         string             `json:"-"` // Not in API response
}

// GetRollingRelease returns the rolling release for a given project.
func (c *Client) GetRollingRelease(ctx context.Context, projectID, teamID string) (RollingReleaseResponse, error) {
	...
	if err != nil {
		return RollingReleaseResponse{}, fmt.Errorf("error getting rolling-release: %w", err)
	}
...
// UpdateRollingReleaseRequest defines the information that needs to be passed to Vercel in order to update a rolling release.
...
func (c *Client) UpdateRollingRelease(ctx context.Context, request UpdateRollingReleaseRequest) (RollingReleaseResponse, error) {
	...
	if request.RollingRelease.Enabled {
		...
		var result RollingReleaseResponse
		err := c.doRequest(clientRequest{
			ctx:    ctx,
			method: "PATCH",
		...
		if err != nil {
			return RollingReleaseResponse{}, fmt.Errorf("error enabling rolling release: %w", err)
		}
		...
		disabledRequest := UpdateRollingReleaseRequest{
			RollingRelease: RollingReleaseInfo{
				Enabled:         false,
				AdvancementType: "",
		...
		if err != nil {
			return RollingReleaseResponse{}, fmt.Errorf("error disabling rolling release: %w", err)
		}
