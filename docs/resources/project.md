...
- `protection_bypass_for_automation` (Boolean, Deprecated) Allow automation services to bypass Deployment Protection on this project when using an HTTP header named `x-vercel-protection-bypass` with a value of the `protection_bypass_for_automation_secret` field. Use the `vercel_project_protection_bypass` resource instead.
