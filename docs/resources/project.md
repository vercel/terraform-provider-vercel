...
- `connect_configurations` (Attributes Set) The list of connections from project environment to Secure Compute network. If omitted, Terraform does not manage existing connections. Set to an empty list to remove all connections. (see [below for nested schema](#nestedatt--connect_configurations))
