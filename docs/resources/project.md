...
- `protection_bypass_for_automation_secret` (String, Sensitive) If `protection_bypass_for_automation` is enabled, optionally set this value to specify the automation bypass secret that should be exposed as `VERCEL_AUTOMATION_BYPASS_SECRET` on deployments. If omitted, a secret will be generated.
- `protection_bypass_for_automation_secrets` (Attributes Set) Authoritatively manages the set of automation bypass secrets for this project. This conflicts with `protection_bypass_for_automation_secret` and requires exactly one entry with `is_env_var = true`. (see [below for nested schema](#nestedatt--protection_bypass_for_automation_secrets))
...
- `is_env_var` (Boolean) Whether this secret should be exposed as `VERCEL_AUTOMATION_BYPASS_SECRET` on deployments.
- `secret` (String, Sensitive) The 32 character automation bypass secret.

Optional:

- `note` (String) An optional note to display alongside this bypass in the Vercel UI.
