...
- `import_key_id` (String) The key ID (`kid`) to assign to the imported key. Only valid when `import_key` is set. Changing this forces a new signing key.
...
- `id` (String) The server-minted addressable ID of the signing key. For keys generated by Vercel this is also the JWT/JWKS `kid`; for an imported key the `kid` is the value supplied via `import_key_id` and may differ from this addressable ID.
