...
func TestAcc_ProjectConnectConfigurations(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckTypeSetElemNestedAttrs("vercel_project.test", "connect_configurations.*", map[string]string{
						"env_id":                   "production",
			...
			{
				Config: cfg(testAccProjectConfigWithConnectConfigurations(projectSuffix, connectConfigurationID, true, false)),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccProjectExists(testClient(t), "vercel_project.test", testTeam(t)),
					resource.TestCheckResourceAttr("vercel_project.test", "connect_configurations.#", "1"),
					resource.TestCheckTypeSetElemNestedAttrs("vercel_project.test", "connect_configurations.*", map[string]string{
			...
			{
				Config: cfg(testAccProjectConfigWithoutConnectConfigurations(projectSuffix)),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccProjectExists(testClient(t), "vercel_project.test", testTeam(t)),
					resource.TestCheckResourceAttr("vercel_project.test", "connect_configurations.#", "0"),
				),
			},
		},
	})
...
func testAccProjectConfigWithConnectConfigurations(projectSuffix, connectConfigurationID string, passive, buildsEnabled bool) string {
	return fmt.Sprintf(`
resource "vercel_project" "test" {
...
`, projectSuffix, connectConfigurationID, passive, buildsEnabled)
}

func testAccProjectConfigWithoutConnectConfigurations(projectSuffix string) string {
	return fmt.Sprintf(`
resource "vercel_project" "test" {
  name = "test-acc-connect-config-%s"
  connect_configurations = []
